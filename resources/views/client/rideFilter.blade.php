        ...
        <div class="row">
            ...
            <div class="col-12 col-md-8 p-4">
                @foreach ($ridefilterData as $ridefilter)
                    ...
                    <a href="{{ route('user.seatPosition', ['id' => $ridefilter->id]) }}">
                        <div class="card mb-5">
                            <div class="card-header d-flex justify-content-between text-center">
                                <div>
                                    <h5 class="text-danger">
                                        {{ Carbon\Carbon::parse($ridefilter->timeStart)->format('H:i') }}</h5>
                                ...
                                <div>
                                    <h5 class="text-danger">{{ Carbon\Carbon::parse($ridefilter->timeEnd)->format('H:i') }}
                                    </h5>
                            ...
                            <div class="card-body">
                                <ul class="nav nav-tabs tab-custom" id="myTab-{{ $ridefilter->id }}" role="tablist">
                                    <li class="nav-item">
                                        <a class="nav-link active" id="seat-tab-{{ $ridefilter->id }}" data-toggle="tab"
                                            href="#seat" role="tab" aria-controls="seat" aria-selected="true">Chọn
                                            ghế</a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link" id="schedule-tab-{{ $ridefilter->id }}" data-toggle="tab"
                                            href="#schedule" role="tab" aria-controls="schedule"
                                            aria-selected="false">Lịch trình</a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link" id="transfer-tab-{{ $ridefilter->id }}" data-toggle="tab"
                                            href="#transfer" role="tab" aria-controls="transfer"
                                            aria-selected="false">Trung chuyển</a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link" id="policy-tab-{{ $ridefilter->id }}" data-toggle="tab"
                                            href="#policy" role="tab" aria-controls="policy" aria-selected="false">Chính
                                            sách</a>
                                    </li>
                                </ul>
                                <div class="tab-content mt-3" id="myTabContent-{{ $ridefilter->id }}">
                                    ...
                                    <div class="tab-pane fade" id="schedule-{{ $ridefilter->id }}" role="tabpanel"
                                        aria-labelledby="schedule-tab">
                                        <p>{{ Carbon\Carbon::parse($ridefilter->timeStart)->format('H:i') }} -
                                            {{ $ridefilter->goto }}</p>
                                        <p>{{ Carbon\Carbon::parse($ridefilter->timeEnd)->format('H:i') }} -
                                            {{ $ridefilter->gofrom }}</p>
                                    </div>
                                    <div class="tab-pane fade" id="transfer-{{ $ridefilter->id }}" role="tabpanel"
                                        ...
                                        <p> - Hẻm nhỏ xe không quay đầu được : Xe trung chuyển sẽ đón Khách đầu hẻm/ đầu
                                            đường.</p>
                                        <p>- Khu vực có biển cấm dừng đỗ xe không đón được : Xe trung chuyển sẽ đón tại vị
                                            trí gần nhất có thể.</p>
                                        <p>- Hành lý : Hành lý nhỏ gọn dưới 20 kg, không vận chuyển kèm động vật , thú cưng.
                                        </p>
                                    </div>
                                    <div class="tab-pane fade" id="policy-{{ $ridefilter->id }}" role="tabpanel"
                                        ...
                                        Yêu cầu khi lên xe
                                        Có mặt tại Văn phòng/Bến xe (Địa điểm xe đón trực tiếp) trước 30 phút để làm thủ tục
                                        lên xe (đối với ngày lễ tết cần ra trước 60 phút).
                                        ...
                                        Trong trường hợp trẻ em không thoả 1 trong 3 tiêu chí trên sẽ mua 01 vé tương đương
                                        với người lớn.
                                        ...
                                        Vé đón đường
                                        Trường hợp có nhu cầu lên xe dọc đường, Quý khách vui lòng liên hệ tổng đài 19006067
                                        để đăng kí trước ít nhất 2 tiếng so với giờ xe khởi hành và vui lòng chuẩn bị hành
                                        lý nhỏ gọn (tối đa 20kg).
