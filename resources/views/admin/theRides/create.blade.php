            ...
            <div class="card-body">
                <form action="{{ route('dashboard.theride.store') }}" method="post">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="datetime-local" class="form-control" id="time_to_go" name="time_to_go"
                            value="{{ old('time_to_go') }}">
                        @error('time_to_go')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="row mb-3">
                        <div class="col-lg-6">
                            ...
                            <input type="datetime-local" class="form-control" id="estimated_departure_time"
                                name="estimated_departure_time" value="{{ old('estimated_departure_time') }}">
                        ...
                        <div class="col-lg-6">
                            ...
                            <input type="datetime-local" class="form-control" id="estimated_arrival_time"
                                name="estimated_arrival_time" value="{{ old('estimated_arrival_time') }}">
                    ...
                    <div class="mb-3">
                        <label for="time_to_go" class="form-label">Tuyến Xe</label>
                        <select class="form-select" id="nameCars" name="carriage_way_id">
                            @foreach ($carriageWays as $row)
                                <option value="{{ $row->id }}"
                                    {{ old('carriage_way_id') == $row->id ? 'selected' : '' }}>{{ $row->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
