                ...
                <form action="{{ route('seat.store') }}" method="post">
                    ...
                    <div class="row">
                        <div class="col-lg-6 mb-3">
                            ...
                            <input type="text" class="form-control" id="name" name="name" value="{{ old('name') }}" placeholder="Nhập tên ghế">
                            @error('name')
                        ...
                        <div class="col-lg-6 mb-3">
                            ...
                            <input type="number" class="form-control" id="price" name="price" value="{{ old('price') }}" placeholder="Nhập giá ghế">
                            @error('price')
                            <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-6 mb-3">
                            ...
                            <select class="form-select" id="nameCars" name="cars_id">
                                @foreach($cars as $row)
                                    <option value="{{ $row->id }}" {{ old('nameCars') == $row->id ? 'selected' : '' }}>{{ $row->license_plates}}</option>
                                @endforeach
                            </select>
                            @error('nameCars')
                            <div class="text-danger">{{ $message }}</div>
